...
export function WorkoutAssignmentForm({ clientId, onClose, onSuccess }: WorkoutAssignmentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/workouts/assignments', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Failed to assign workout plan');
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="premium-card rounded-2xl p-8 max-w-xl w-full border border-gray-800/50">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">Assign Workout Plan</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                ...
                {plans.map((plan) => (
                  <option key={plan.id} value={plan.id}>
                    {plan.title} ({plan.level} - {plan.goal_type.replace('_', ' ')})
                  </option>
                ))}
              </select>
            )}
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="submit"
              disabled={isSaving || !formData.workout_plan_id}
