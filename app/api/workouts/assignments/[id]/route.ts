...
import { updateClientWorkoutAssignmentSchema } from '@/lib/validations';
...
export async function PATCH(
  ...
  try {
    ...
    const validatedData = updateClientWorkoutAssignmentSchema.parse(body);
    ...
    console.error('Error in PATCH /api/workouts/assignments/[id]:', error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error in DELETE /api/workouts/assignments/[id]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
